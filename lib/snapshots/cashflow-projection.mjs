...
export function availableCashflowYears(tables) {
  ...
  for (const row of tables.CashflowManualBalance ?? []) years.add(Number(row.year));
...
export function projectCashflow(tables, year, now = new Date()) {
  ...
  const manualBalance = Array.from({ length: count }, () => null);
  ...
  for (const row of tables.CashflowManualBalance ?? []) {
    if (Number(row.year) !== year) continue;
    const index = Number(row.week) - 1;
    if (index < 0 || index >= count) continue;
    manualBalance[index] = row.amount == null ? null : Number(row.amount);
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    const balanceStartDP = i === 0 ? startBalance : manualBalance[i - 1] ?? previousDP;
    previousDP = balanceStartDP + chargeTotal[i] - dpExpense;
